import {$, api, emptyState, escapeHtml, fmtBytes, navigate, setMessages, state, stateIndicator} from './core.js';
...
export async function pickProject() { const generation=beginProjectSelection(); try { const result = await api('/api/projects/pick-folder',{method:'POST'}); if (!isCurrentProjectSelection(generation)||result.cancelled)return result; await refreshProjects(); if(!isCurrentProjectSelection(generation))return result; $('project-select').value=result.project.id; syncWorkspaceAccess(); $('route-status').textContent=`Folder ready - ${result.project.name}`; $('project-dialog').close(); return result; } catch(error){if(isCurrentProjectSelection(generation))alert(error.message);} }
export async function createProject() { const name=$('new-project-name').value.trim(); if(!name)return; try{const created=await api('/api/projects/create',{method:'POST',body:JSON.stringify({name,approved:true})});await loadProject(created.project);$('create-dialog').close();$('new-project-name').value='';}catch(error){alert(error.message);} }
...
export async function send(replay=null){
  ...
  try{
    ...
    const explanation=readable[error.code]||error.message||'The request could not be completed.';
