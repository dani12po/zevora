import {$, api, badge, emptyState, escapeHtml, exposeHandlers, pageWrap, setPanel, userErrorMessage} from './core.js?v=20260819-3';

const customManifestCache = new Map();
const providerHealthCache = new Map();

const LABELS = {local:'Zevora Local AI',local_remote:'Remote Lexi',openai:'OpenAI',xai:'xAI (Grok)',nvidia:'NVIDIA NIM',deepseek:'DeepSeek',gemini:'Google Gemini',anthropic:'Anthropic'};
const DESCRIPTIONS = {
  local:'Lexi Llama 3 8B Q5_K_M - embedded llama.cpp GGUF - no API key required',
  local_remote:'Lexi on an external llama.cpp server - OpenAI-compatible - needs base URL',
...
function providerCard(provider, statusMap, modelCounts) {
  ...
  if (provider.provider === 'local') {
    ...
    return `<div class="card provider-shell"><div class="provider-card">
      <div class="provider-card-name"><span>${LABELS.local}</span>${healthDot('local', provider)}${healthRefreshButton('local', false, true)} <span id="health-badge-local">${stateBadge}</span></div><div class="technical-text">${escapeHtml(runtime.display_name || '')} - ${runtime.runtime || 'llamacpp'} - ${runtime.deployment || 'embedded'}</div>
      <div class="provider-description">${DESCRIPTIONS.local}</div><div class="card-grid provider-stats">
        <div class="card-sm card"><div class="card-lbl">Model</div><div class="card-val technical-text">${escapeHtml(runtime.model_id || provider.default_model || 'lexi')}</div></div>
        <div class="card-sm card"><div class="card-lbl">Quant</div><div class="card-val">${escapeHtml(runtime.quantization || '-')}</div></div>
        ...
        <div class="card-sm card"><div class="card-lbl">State</div><div class="card-val">${escapeHtml(runtime.state || '-')}</div></div>
        ...
        <div class="card-sm card"><div class="card-lbl">Remote Lexi</div><div class="card-val">${provider.remote_status?.base_url ? `${escapeHtml(provider.remote_status.last_error || 'configured')} - ${provider.remote_status.last_latency_ms ?? '-'} ms` : 'Not configured'}</div></div>
      </div><div class="provider-path technical-text">${escapeHtml(runtime.model_repository || '')}</div><div class="provider-path technical-text">${escapeHtml(runtime.model_path || '')}</div>
    </div></div>`;
  }
  ...
  return `<div class="card provider-shell ${custom ? 'provider-custom' : ''}"><div class="provider-card">
    ...
    <div class="provider-summary"><span>${modelCount} models</span><label class="toggle" title="Enabled"><input type="checkbox" id="toggle-${id}" aria-label="Enable provider" ${provider.enabled ? 'checked' : ''} onchange="${custom ? `toggleCustomProvider('${id}', this.checked)` : `markProviderDirty('${id}')`}"><span class="toggle-slider"></span></label></div>
    ...
    <div class="provider-fields"><div class="provider-field"><label>API Key</label><span class="key-display" id="key-display-${id}">${provider.key_set ? `********${escapeHtml(provider.key_masked.slice(-4))}` : 'Not set'}</span><button class="btn-sm" onclick="${custom ? `toggleCustomKeyEdit('${id}')` : `toggleKeyEdit('${id}')`}">Edit</button></div>
      ...
      <div class="provider-field"><label>Priority</label><input type="number" id="prio-${id}" value="${provider.routing_priority ?? 50}" min="0" max="100" oninput="markProviderDirty('${id}')"><span></span></div>
...
export async function saveCustomProviderCard(id) {
  ...
  const payload = {
    ...
    routing_priority: Math.min(100, Math.max(0, Number.parseInt($(`prio-${id}`)?.value, 10) || 50)),
...
export async function saveProvider(id) { const button=$(`save-btn-${id}`), key=$(`key-input-${id}`), priority=Number.parseInt($(`prio-${id}`)?.value,10); const clampedPriority=Number.isNaN(priority)?null:Math.min(100,Math.max(0,priority)); button.classList.add('btn-loading'); try { const result=await api('/api/providers/config',{method:'POST',body:JSON.stringify({provider:id,base_url:$(`url-${id}`)?.value||null,default_model:$(`model-${id}`)?.value?.trim()||null,routing_priority:clampedPriority,enabled:$(`toggle-${id}`)?.checked??null,supports_vision:$(`vision-${id}`)?.checked??null,api_key:key?.value?.trim()||null})}); await renderProviders(); const message=verificationMessage(result); showProviderSave(id,message.text,message.tone); button.disabled=true; if(key){key.value='';cancelKeyEdit(id);} } catch(error){showProviderSave(id,userErrorMessage(error),'error');} finally{button.classList.remove('btn-loading');} }
