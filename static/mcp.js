...
export async function renderMCP() {
  ...
  document.querySelectorAll('[data-mcp-tool]').forEach(input => {
    input.onchange = () => setMCPToolEnabled(input.dataset.mcpTool, input);
  });
}
...
export async function setMCPToolEnabled(tool, input) {
  ...
  try {
    ...
    if (status) status.textContent = userErrorMessage(error, 'Tool update failed.');
  } finally {
